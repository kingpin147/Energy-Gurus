...
export async function DashboardReviewList({ targetId, targetType }: { targetId: string, targetType: string }) {
    ...
    if (allReviews.length === 0) {
        return (
            <div className="py-12 text-center bg-paper/5 rounded-3xl border-2 border-dashed">
                <Star className="w-12 h-12 text-slate-custom mx-auto mb-4 opacity-10" />
                <p className="text-slate-custom font-medium">No reviews received yet.</p>
            </div>
        );
    }
            ...
            {allReviews.map((review) => (
                <Card key={review.id} className="border-none shadow-sm bg-white rounded-3xl overflow-hidden">
                    <CardContent className="p-8">
                        ...
                        <p className="text-slate-custom leading-relaxed italic mb-6">
                            "{review.comment}"
                        </p>

                        {review.reply ? (
                            <div className="p-6 bg-amber/5 text-ink rounded-2xl border-l-4 border-amber space-y-2">
                                <p className="text-[10px] font-black uppercase tracking-widest text-amber opacity-60">Your Reply</p>
                                <p className="text-sm text-graphite leading-relaxed">{review.reply}</p>
                            </div>
                        ) : (
                            <form action={replyToReview} className="space-y-3 pt-4 border-t">
                                ...
                                <div className="flex gap-2">
                                    <textarea
                                        ...
                                        className="flex-1 border rounded-xl p-3 text-sm bg-paper/5 focus:ring-2 focus:ring-primary outline-none"
                                        required
                                    />
