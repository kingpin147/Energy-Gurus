...
import { Globe, Mail, Star, ShieldCheck, ArrowLeft, Image as ImageIcon, Facebook, Twitter, Instagram, Linkedin, Zap, Shield, Award, MapPin, Building2, Package, CheckCircle2, Info, MessageSquare, ExternalLink, HeadphonesIcon } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/20 pb-32">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-0 right-0 w-[40%] h-[30%] bg-accent/5 rounded-full blur-[120px]" />
      </div>

      {/* Navigation */}
      <nav className="container mx-auto px-6 py-10 relative z-10">
        <Button variant="ghost" className="group p-0 hover:bg-transparent text-muted-foreground hover:text-primary transition-all font-black uppercase tracking-[0.2em] text-[10px]" asChild>
          <Link href="/brands">
      ...
      <section className="container mx-auto px-6 mb-24 relative z-10">
        <div className="relative rounded-[4rem] overflow-hidden border border-border/50 premium-shadow bg-white/40 backdrop-blur-3xl p-10 md:p-20">
          <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-accent/5 opacity-50" />

          <div className="flex flex-col items-center text-center gap-16 relative z-10">
            {/* Brand Identity */}
            <div className="relative group">
              <div className="absolute inset-0 bg-primary/20 rounded-[3.5rem] blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
            ...
            <div className="flex-1 space-y-8 max-w-4xl">
              <div className="space-y-4">
                <div className="flex flex-wrap items-center justify-center gap-3">
                  {brand.isVerified && (
                    <span className="px-5 py-2 bg-primary/10 text-primary rounded-full text-[10px] font-black uppercase tracking-[0.2em] border border-primary/20 flex items-center gap-2">
                      <ShieldCheck className="w-4 h-4" /> Tier-1 Verified
                    </span>
                  )}
                  <span className="px-5 py-2 bg-accent/5 text-accent-foreground rounded-full text-[10px] font-black uppercase tracking-[0.2em] border border-border/50">
                    Global Manufacturer
                  </span>
                </div>

                <h1 className="text-6xl md:text-9xl font-black tracking-tighter leading-[0.8] text-foreground">
                  {brand.brandName}
                </h1>

                <div className="flex flex-wrap items-center justify-center gap-6 pt-4">
                  {count > 0 ? (
                    <div className="flex items-center gap-4 px-6 py-3 bg-white/50 rounded-2xl border border-border/50 shadow-sm">
                      <div className="flex items-center text-yellow-500">
                        <Star className="w-6 h-6 fill-current" />
                        <span className="ml-2 text-3xl font-black text-foreground">{rating?.toFixed(1)}</span>
                      </div>
                    ...
                    <div className="text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em] bg-white/30 px-6 py-3 rounded-2xl border border-dashed border-border/50">
                      Market Rating: N/A
                    </div>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
          ...
          <div className="lg:col-span-8 space-y-32">
            ...
            <section className="space-y-12">
              ...
              <div className="bg-white/50 backdrop-blur-2xl p-12 md:p-20 rounded-[4rem] border border-border/50 shadow-sm relative overflow-hidden group">
                <div className="absolute -top-20 -right-20 w-64 h-64 bg-primary/5 rounded-full blur-3xl transition-transform duration-1000 group-hover:scale-150" />
                <p className="text-2xl md:text-3xl leading-relaxed text-foreground font-medium italic opacity-90 relative z-10">
            ...
            <section className="space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {certifications.map((cert) => (
                  <div key={cert.id} className="p-10 bg-white/50 backdrop-blur-xl border border-border/50 rounded-[3rem] flex flex-col gap-6 shadow-sm group hover:border-primary transition-all duration-700">
                    <div className="flex items-center gap-6">
                      <div className="w-16 h-16 bg-primary/5 rounded-2xl flex items-center justify-center border border-primary/10 group-hover:bg-primary transition-colors duration-700">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">{cert.issuingBody || "Verified Agency"}</p>
                      </div>
                    </div>
                    {cert.expiryDate && (
                      <div className="pt-6 border-t border-border/30 flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                        <span className="opacity-40">VALID UNTIL</span>
                        <span className="text-primary">{new Date(cert.expiryDate).toLocaleDateString()}</span>
                      </div>
                    )}
                  </div>
                ))}
                ...
                {certifications.length === 0 && (
                  <div className="col-span-full py-16 text-center bg-accent/5 rounded-[3rem] border border-dashed border-accent/20">
                    <Shield className="w-12 h-12 text-accent/10 mx-auto mb-4" />
                    <p className="text-sm font-black text-accent-foreground/30 uppercase tracking-[0.3em]">Compliance Portfolio: N/A</p>
                  </div>
                )}
              </div>
            </section>
            ...
            <section className="space-y-12">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                <div className="space-y-8">
                  <div className="bg-primary/5 p-12 rounded-[3.5rem] border border-primary/10 premium-shadow">
                    <h3 className="text-2xl font-black uppercase tracking-tighter mb-10 flex items-center gap-4">
                      <Award className="w-6 h-6 text-primary" /> Audit Experience
                    </h3>
                    <ReviewForm targetId={id} targetType="brand" />
                  </div>
                </div>
                <div className="space-y-10">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30 border-b border-border/50 pb-6">Verified Global Audits</h3>
          ...
          <div className="lg:col-span-4">
            <aside className="sticky top-12 space-y-10">
                    ...
                    <div className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <p className="text-xl font-black flex items-center gap-3">
                          <Building2 className="w-5 h-5 text-primary" /> {brand.countryHead || "N/A"}
                        </p>
                      </div>
                      <div className="space-y-2">
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-30 leading-none">Customer Care</p>
                        <p className="text-xl font-black flex items-center gap-3 text-primary">
                      ...
                      <div className="space-y-4">
                        {(brand.reps as { name: string; designation: string }[]).map((rep, i) => (
                          <div key={i} className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                            <div className="w-10 h-10 bg-primary/20 rounded-xl flex items-center justify-center font-black text-primary uppercase">
                              {rep.name.charAt(0)}
                            </div>
                            <div>
                  ...
                  <div className="space-y-4 pt-10 border-t border-white/5">
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button className="w-full h-16 rounded-[1.8rem] font-black text-lg bg-primary text-white hover:bg-primary/90 gap-4 transition-all hover:scale-[1.02] shadow-xl shadow-primary/20">
                          <Mail className="w-6 h-6" /> InMail Contact
                        </Button>
                      </DialogTrigger>
                      <DialogContent className="sm:max-w-[500px] rounded-[3rem] p-0 overflow-hidden border-none">
                        <div className="bg-[#0F172A] p-10 text-white">
                          <h3 className="text-3xl font-black tracking-tighter">Direct Inquiry</h3>
                          <p className="text-white/40 text-sm font-medium">Send a verified message to {brand.brandName} headquarters.</p>
                        </div>
              ...
              <div className="p-12 rounded-[4rem] bg-accent/5 border border-accent/10 relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-40 h-40 bg-accent/5 rounded-bl-full group-hover:scale-125 transition-transform duration-1000" />
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-accent-foreground mb-6 flex items-center gap-2">
                  <Info className="w-4 h-4" /> Supply Chain Metrics
                </h4>
                <div className="space-y-6">
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                    <span className="opacity-40">System Integration</span>
                    <span className="text-accent-foreground">Global Standard</span>
                  </div>
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                    <span className="opacity-40">Market Presence</span>
                    <span className="text-accent-foreground">Tier-1 Rank</span>
                  </div>
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
